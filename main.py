...
@bot.event
async def on_ready():
    print(f"DiscoBot started | We got filters!")
...
class DoneTaskButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Завершить задачу', row=0, custom_id='done_task', style=discord.ButtonStyle.success)

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_message("Введите ID задачи для ЗАВЕРШЕНИЯ (неск. - через пробел)")
        def check(m):
            return m.author.id == interaction.user.id and m.channel.id == interaction.channel.id
        message = await bot.wait_for('message', check=check)
        ...
        for task_id in task_ids:
            try:
                subprocess.run(["task", task_id, "done"], check=True)
            except subprocess.CalledProcessError:
                await interaction.followup.send(f'Не удалось завершить задачу {task_id}. Проверьте ID задачи.')

        await interaction.followup.send(f'Задачи {", ".join(task_ids)} завершены.')

#УДАЛЕНИЕ ЗАДАЧ
class DeleteTasksButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Удаление задач', row=0, custom_id='delete_task', style=discord.ButtonStyle.danger)

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_message("Введите ID задачи для УДАЛЕНИЯ (неск. - через пробел)")
        def check(m):
            return m.author.id == interaction.user.id and m.channel.id == interaction.channel.id
        message = await bot.wait_for('message', check=check)
...
class ListTasksButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Список всех задач', row=1, custom_id='list_tasks', style=discord.ButtonStyle.primary)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.guild_permissions.administrator:
            ...
            await interaction.response.send_message('Извините, но эта команда доступна только администраторам.')
...
class CompletedTasksButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Выполненные задачи', row=1, custom_id='completed_tasks', style=discord.ButtonStyle.primary)

    async def callback(self, interaction: discord.Interaction):
        # Проверка на админа
        if interaction.user.guild_permissions.administrator:
            completed_tasks = subprocess.check_output(["task", "completed"])
            completed_tasks = completed_tasks.decode('utf-8')
            completed_tasks = f'```\n{completed_tasks}\n```'
            await interaction.response.send_message(f'Вот ваши выполненные задачи: {completed_tasks}')
        else:
            await interaction.response.send_message('Извините, но эта команда доступна только администраторам.')
...
class AddProjectButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Добавить проект', row=2, custom_id='add_project', style=discord.ButtonStyle.secondary)
...
class AddTagButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Добавить тег', row=2, custom_id='add_tag', style=discord.ButtonStyle.secondary)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(f'Тег {tag} добавлен в задачу {task_id}.')

#Фильтр по проекту
class FilterByProjectButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Фильтр по проекту', row=2, custom_id='filter_by_project', style=discord.ButtonStyle.secondary)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Введите интересующий проект")
        def check(m):
            return m.author.id == interaction.user.id and m.channel.id == interaction.channel.id
        message = await bot.wait_for('message', check=check)
        ...
        try:
            task_list = subprocess.check_output(["task", "list", "project:{}".format(project_name)], stderr=subprocess.STDOUT)
            task_list = task_list.decode('utf-8')
            task_list = f'```\n{task_list}\n```'
            await interaction.followup.send(f'Задачи в проекте {project_name}: \n{task_list}')
        except subprocess.CalledProcessError:
...
class FilterByTagButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label='Фильтр по тегу', row=2, custom_id='filter_by_tag', style=discord.ButtonStyle.secondary)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Введите интересующий тег")
        def check(m):
            return m.author.id == interaction.user.id and m.channel.id == interaction.channel.id
        message = await bot.wait_for('message', check=check)
